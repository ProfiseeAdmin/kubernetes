...
if [ -n "$SECRET_PURVIEW_ARN" ]; then
  ...
  PURVIEW_URL=$(echo "$PURVIEW_JSON" | jq -r '.atlas_endpoint // .url // empty')
  PURVIEW_COLLECTION_ID=$(echo "$PURVIEW_JSON" | jq -r '.collection_id // .collectionId // empty')
  PURVIEW_TENANT_ID=$(echo "$PURVIEW_JSON" | jq -r '.tenant_id // .tenantId // empty')
  PURVIEW_CLIENT_ID=$(echo "$PURVIEW_JSON" | jq -r '.client_id // .clientId // empty')
  PURVIEW_CLIENT_SECRET=$(echo "$PURVIEW_JSON" | jq -r '.client_secret // .clientSecret // empty')
...
JSON
    ...
    if [ "$APP_DEPLOY_ENABLED" = "true" ]; then
      if [ -z "$lb_host" ]; then
        ...
        if [ -n "$PURVIEW_URL" ]; then replace_settings_placeholder /tmp/Settings.yaml '$PURVIEWURL' "$PURVIEW_URL"; fi

...
if (-not $ClusterArn) { $ClusterArn = $outputs.platform_deployer_cluster_arn }
if (-not $TaskDefinitionArn) { $TaskDefinitionArn = $outputs.platform_deployer_task_definition_arn }
if (-not $SubnetIds) { $SubnetIds = ($outputs.private_subnet_ids -join ",") }
if (-not $SecurityGroupIds) { $SecurityGroupIds = $outputs.platform_deployer_security_group_id }

if (-not $ClusterArn) { throw "platform_deployer_cluster_arn not found in outputs_contract." }
if (-not $TaskDefinitionArn) { throw "platform_deployer_task_definition_arn not found in outputs_contract." }
if (-not $SubnetIds) { throw "private_subnet_ids not found in outputs_contract." }
if (-not $SecurityGroupIds) { throw "platform_deployer_security_group_id not found in outputs_contract." }
...
if ($LASTEXITCODE -ne 0) {
  throw "Failed to start Fargate task."
}

Write-Host "Fargate task started."

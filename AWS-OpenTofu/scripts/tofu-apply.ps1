...
try {
  ...
  if ($cloudfrontCfgForPrecheck) {
    $cloudfrontEnabledForPrecheck = Get-PropValue $cloudfrontCfgForPrecheck "enabled"
    $route53CfgForPrecheck = Get-PropValue $cfg "route53"
    if (-not $route53CfgForPrecheck) {
      $cfg | Add-Member -NotePropertyName route53 -NotePropertyValue ([pscustomobject]@{})
      $route53CfgForPrecheck = $cfg.route53
      $cfgChanged = $true
    }

    # Keep Route53 defaults aligned with ACM when CloudFront is selected.
    if ($cloudfrontEnabledForPrecheck -eq $true) {
      $acmCfgForPrecheck = Get-PropValue $cfg "acm"
      $acmDomainForPrecheck = Get-PropValue $acmCfgForPrecheck "domain_name"
      $acmDomainNormalizedForPrecheck = if ($acmDomainForPrecheck) { $acmDomainForPrecheck.ToString().Trim() } else { "" }
      if ($acmDomainNormalizedForPrecheck.StartsWith("*.")) {
        $acmDomainNormalizedForPrecheck = $acmDomainNormalizedForPrecheck.Substring(2)
      }
      $route53NameCurrentForPrecheck = Get-PropValue $route53CfgForPrecheck "record_name"
      $route53NameCurrentNormalizedForPrecheck = if ($route53NameCurrentForPrecheck) { $route53NameCurrentForPrecheck.ToString().Trim().ToLower() } else { "" }
      if (($route53NameCurrentNormalizedForPrecheck -eq "" -or $route53NameCurrentNormalizedForPrecheck -eq "app.example.com") -and $acmDomainNormalizedForPrecheck -ne "") {
        Set-PropValue $route53CfgForPrecheck "record_name" $acmDomainNormalizedForPrecheck
        $cfgChanged = $true
      }

      $acmHostedZoneIdForPrecheck = Get-PropValue $acmCfgForPrecheck "hosted_zone_id"
      $route53ZoneCurrentForPrecheck = Get-PropValue $route53CfgForPrecheck "hosted_zone_id"
      $route53ZoneCurrentNormalizedForPrecheck = if ($route53ZoneCurrentForPrecheck) { $route53ZoneCurrentForPrecheck.ToString().Trim().ToUpper() } else { "" }
      if (($route53ZoneCurrentNormalizedForPrecheck -eq "" -or $route53ZoneCurrentNormalizedForPrecheck -eq "Z1234567890ABC") -and $acmHostedZoneIdForPrecheck) {
        Set-PropValue $route53CfgForPrecheck "hosted_zone_id" $acmHostedZoneIdForPrecheck
        $cfgChanged = $true
      }
    }
    ...
    # Migration safety: if an existing CNAME is present for the app host, keep record_type as CNAME.
    $originForPrecheck = Get-PropValue $cloudfrontCfgForPrecheck "origin_domain_name"
    $originForPrecheckNormalized = if ($originForPrecheck) { $originForPrecheck.ToString().Trim() } else { "" }
    $route53NameForTypePrecheck = Get-PropValue $route53CfgForPrecheck "record_name"
    $route53ZoneForTypePrecheck = Get-PropValue $route53CfgForPrecheck "hosted_zone_id"
    if (
      $cloudfrontEnabledForPrecheck -eq $true -and
      $originForPrecheckNormalized -ne "" -and
      $route53NameForTypePrecheck -and
      $route53ZoneForTypePrecheck -and
      (Get-Command aws -ErrorAction SilentlyContinue)
    ) {
      $lookupNameForPrecheck = $route53NameForTypePrecheck.ToString().Trim().TrimEnd(".").ToLower()
      $lookupNameDotForPrecheck = "$lookupNameForPrecheck."
      $existingTypeForPrecheck = aws route53 list-resource-record-sets `
        --hosted-zone-id $route53ZoneForTypePrecheck `
        --query "ResourceRecordSets[?Name=='$lookupNameDotForPrecheck']|[0].Type" `
        --output text 2>$null
      if ($LASTEXITCODE -eq 0 -and $existingTypeForPrecheck -and $existingTypeForPrecheck -ne "None") {
        $existingTypeNormalizedForPrecheck = $existingTypeForPrecheck.ToString().Trim().ToUpper()
        $currentTypeForPrecheck = Get-PropValue $route53CfgForPrecheck "record_type"
        $currentTypeNormalizedForPrecheck = if ($currentTypeForPrecheck) { $currentTypeForPrecheck.ToString().Trim().ToUpper() } else { "A" }
        if ($existingTypeNormalizedForPrecheck -eq "CNAME" -and $currentTypeNormalizedForPrecheck -ne "CNAME") {
          Set-PropValue $route53CfgForPrecheck "record_type" "CNAME"
          Write-Host ("Detected existing Route53 CNAME for {0}; setting route53.record_type=CNAME before apply." -f $lookupNameForPrecheck)
          $cfgChanged = $true
        }
      }
    }
...
if ($cloudfrontEnabled -eq $true) {
  ...
  if (-not $nlbDns -or $nlbDns -eq "") {
    ...
    $acmCfg = Get-PropValue $cfg "acm"

    $recordName = Get-PropValue $route53Cfg "record_name"
    $recordNameNormalized = if ($recordName) { $recordName.ToString().Trim() } else { "" }
    if ($recordNameNormalized.StartsWith("*.")) {
      $recordNameNormalized = $recordNameNormalized.Substring(2)
    }
    if (-not $recordNameNormalized -or $recordNameNormalized.ToLower() -eq "app.example.com") {
      $acmDomain = Get-PropValue $acmCfg "domain_name"
      $acmDomainNormalized = if ($acmDomain) { $acmDomain.ToString().Trim() } else { "" }
      if ($acmDomainNormalized.StartsWith("*.")) {
        $acmDomainNormalized = $acmDomainNormalized.Substring(2)
      }
      if ($acmDomainNormalized) {
        Set-PropValue $route53Cfg "record_name" $acmDomainNormalized
        $recordNameNormalized = $acmDomainNormalized
        $edgeConfigChanged = $true
        Write-Host ("Route53 record_name defaulted to ACM domain: {0}" -f $acmDomainNormalized)
      }
    }
    $recordName = $recordNameNormalized

    $hostedZoneId = Get-PropValue $route53Cfg "hosted_zone_id"
    $hostedZoneIdNormalized = if ($hostedZoneId) { $hostedZoneId.ToString().Trim() } else { "" }
    if (-not $hostedZoneIdNormalized -or $hostedZoneIdNormalized.ToUpper() -eq "Z1234567890ABC") {
      $acmHostedZoneId = Get-PropValue $acmCfg "hosted_zone_id"
      if ($acmHostedZoneId) {
        Set-PropValue $route53Cfg "hosted_zone_id" $acmHostedZoneId
        $hostedZoneIdNormalized = $acmHostedZoneId
        $edgeConfigChanged = $true
        Write-Host ("Route53 hosted_zone_id defaulted to ACM hosted zone ID: {0}" -f $acmHostedZoneId)
      }
    }
    $hostedZoneId = $hostedZoneIdNormalized

    $recordType = Get-PropValue $route53Cfg "record_type"
    $recordTypeNormalized = if ($recordType) { $recordType.ToString().Trim().ToUpper() } else { "A" }
    if (-not $recordType -or $recordType.ToString().Trim() -eq "") {
      Set-PropValue $route53Cfg "record_type" $recordTypeNormalized
      $edgeConfigChanged = $true
    }

    if ($recordName -and $hostedZoneId) {
      $recordLookup = $recordName.ToString().Trim().TrimEnd(".").ToLower()
      if ($recordLookup) {
        $recordLookupDot = "$recordLookup."
        $existingType = aws route53 list-resource-record-sets `
          --hosted-zone-id $hostedZoneId `
          --query "ResourceRecordSets[?Name=='$recordLookupDot']|[0].Type" `
          --output text 2>$null

        if ($LASTEXITCODE -eq 0 -and $existingType -and $existingType -ne "None") {
          $existingTypeNormalized = $existingType.ToString().Trim().ToUpper()
          if ($existingTypeNormalized -eq "CNAME" -and $recordTypeNormalized -ne "CNAME") {
            Set-PropValue $route53Cfg "record_type" "CNAME"
            $recordTypeNormalized = "CNAME"
            $edgeConfigChanged = $true
            Write-Host ("Detected existing Route53 CNAME for {0}; using record_type CNAME for CloudFront DNS wiring." -f $recordLookup)
          }
        }
      }
    }

    if (-not $recordName -or $recordName -eq "" -or -not $hostedZoneId -or $hostedZoneId -eq "") {
      throw "CloudFront is enabled but route53.record_name/hosted_zone_id are missing. Set ACM and Route53 values in config.auto.tfvars.json."
    }
    ...
    if ($edgeConfigChanged) {
      ...
      Write-Host "CloudFront config unchanged; applying edge stack to reconcile potential Route53 drift from profisee_deploy."
    }
